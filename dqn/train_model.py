...
def train_dqn(env, Q, Q_new, LR, balance):
    ...
    try:
        epoch, Q, optimizer = load(Q, optimizer)
        print("file loaded")
    ...
    for epoch in range(epoch_num):
        ...
        while (not done and step < step_max):
            ...
            if (step % 5 == 0):
                    ...
                    if total_step % train_freq == 0:
                        ...
                        for i in memory_idx[::batch_size]:
                            ...
                            for j in range(batch_size): # update target by using bellman equation
                                target[j, b_action[j] - 1] = torch.from_numpy(b_reward[j] + gamma * maxQ[j] * (not b_done[j])).float().to(device)
                            Q.zero_grad()
                ...
                if epsilon > epsilon_min and total_step > start_reduce_epsilon: # Decay Epsilon
